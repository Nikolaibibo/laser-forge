...
- ✅ **Task 5** (editable dual-layer text) — `Artwork` gains `labels`/`source`; blueprint populates them; `svgExport` `editableText` (default true) emits `<metadata id="lf-blueprint">` (CDATA JSON) + `plot` layer + hidden `text` layer (one `<text>` per shown field, XML-escaped). `src/util/blueprintMeta.ts` = serialize/parse. Round-trip wired in `Inspector.onFile` (imported SVG with our metadata → restore generator+params, not loaded as motif; motif itself not persisted). **Plot path (AxiDrawPanel) forces `editableText:false`** so axicli/bridge only see paths. Verified: plot paths unchanged, round-trip params deep-equal, flat mode plain.
- ✅ **Task 6** (vpype pre-pass in bridge) — `bridge/bridge.py` gains `maybe_vpype()` (env-gated `LF_VPYPE=1`, bin `LF_VPYPE_BIN` default `~/.venvs/vpype/bin/vpype`), called in `plot_svg` before `prep_svg`. Runs `read → linemerge 0.1mm → linesort → linesimplify 0.05mm → write`; any failure/missing-binary falls back to the raw SVG (never blocks a plot). **Note:** vpype writes `<polyline>`/`<line>` (not `<path>`) — `prep_svg` wraps them fine, axicli plots them. Mac-verified: RP-1357 LF export 1409→342 geometry elements, prep_svg preserves them, disabled=identity, missing-binary=raw fallback. **⏳ Pending: install vpype in a venv on the gimbal-Pi + `LF_VPYPE=1` + real E2E plot (needs SSH).**
- ✅ **Task 7** (tests) — `scripts/blueprint-test.ts` now has 18 checks: added per-field sizing, show toggles, textAlign, frameStyle rect counts, pen-width guard (warns/clears + geometry-invariant), editable dual-layer + round-trip (deep-equal, XML-escape, flat-is-plain), and page-format presets.

**All 7 tasks landed. Only open item: install vpype on the gimbal-Pi + real E2E plot with `LF_VPYPE=1` (needs SSH).**
